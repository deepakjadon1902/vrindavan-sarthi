...
import {
  ...
  Bell, UserCog, Landmark, ShieldCheck, FileText, CalendarDays,
} from 'lucide-react';
...
const sidebarLinks = [
  ...
  { name: 'Partner T&C', path: '/partner/terms', icon: FileText },
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="panel-hero min-h-20 border-b border-brand-gold/20 flex items-center justify-between px-4 lg:px-8">
          ...
          <BookingAlarmManager
            token={token}
            user={user}
            enabled={user?.role === 'partner' && user?.partnerStatus === 'approved'}
            viewPath="/partner/bookings"
            onNewBooking={() => void fetchPartnerBookings()}
          />
        </header>
