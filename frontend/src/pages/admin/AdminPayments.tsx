...
const AdminPayments = () => {
  ...
  const { adminOrders, fetchAllOrders, verifyOrderPayment, rejectOrderPayment, isLoadingOrders } = useProductStore();
  ...
  useEffect(() => {
    fetchAllOrders();
    fetchAllBookings();
  }, [fetchAllBookings, fetchAllOrders]);
  ...
  const handleVerify = async (p: UnifiedPayment) => {
    if (p.source === 'booking') {
      const res = await verifyPayment(p.recordId);
      if (res.success) toast.success('Payment verified & booking confirmed!');
      else toast.error(res.error || 'Verify failed');
      return;
    }
    const res = await verifyOrderPayment(p.recordId);
    if (res.success) toast.success('Payment verified & order confirmed!');
    else toast.error(res.error || 'Verify failed');
  };

  const handleReject = async (p: UnifiedPayment) => {
    if (p.source === 'booking') {
      const res = await rejectPayment(p.recordId);
      if (res.success) toast.error('Payment rejected & booking cancelled');
      else toast.error(res.error || 'Reject failed');
      return;
    }
    const res = await rejectOrderPayment(p.recordId);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2">
        {(['all', 'hotel', 'room', 'cab', 'tour', 'product'] as const).map((t) => (
          <button key={t} onClick={() => setTypeFilter(t)} className={`px-4 py-2 rounded-lg font-body text-sm capitalize transition-colors ${typeFilter === t ? 'bg-brand-gold text-foreground' : 'bg-card border border-border hover:bg-muted'}`}>
            {t}
          </button>
        ))}
      </div>
      ...
      {(isLoadingOrders || isLoading) && unified.length === 0 ? (
        <div className="bg-card rounded-xl border border-border p-12 text-center">
          <CreditCard size={48} className="mx-auto mb-4 text-muted-foreground/30" />
          <p className="font-heading text-xl text-foreground mb-2">Loading Payments…</p>
