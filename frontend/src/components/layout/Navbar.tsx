...
const navLinks = [
  { name: 'Stays', path: '/hotels', icon: Building2 },
  { name: 'Rooms', path: '/rooms', icon: BedDouble },
  { name: 'Cabs', path: '/cabs', icon: CarTaxiFront },
  { name: 'Tours', path: '/tours', icon: MapPinned },
  { name: 'Explore Braj', path: '/about', icon: MapPinned },
...
const Navbar = () => {
  ...
  const { shopEnabled } = useSettingsStore((s) => s.settings);
  const visibleNavLinks = navLinks.filter((link) => {
    if (link.path === '/shop' || link.path === '/track-order') return false;
    return true;
  });
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 border-b transition-all duration-300 ${navBg} ${scrolled ? 'border-brand-gold/35' : ''}`}>
        <div className="mx-auto w-full max-w-[1440px] px-3 sm:px-5 xl:px-5 2xl:px-7">
          <div className="grid h-16 grid-cols-[1fr_auto] items-center gap-3 xl:h-20 xl:grid-cols-[88px_minmax(0,1fr)_auto] xl:gap-4 2xl:grid-cols-[110px_minmax(0,1fr)_auto] 2xl:gap-5">
            ...
            <div className={`hidden w-fit min-w-0 justify-self-center items-center justify-center gap-0.5 rounded-xl px-1 py-1 xl:flex 2xl:gap-1 ${navPanelClass}`}>
              {visibleNavLinks.map((link) => {
                ...
                <Link
                  key={link.path}
                  to={link.path}
                  className={`relative inline-flex min-h-10 items-center gap-1.5 whitespace-nowrap rounded-[10px] px-2.5 py-2 font-body text-[13px] font-bold tracking-normal transition-all duration-200 2xl:px-3 ${
                    navItemClass(active)
                  }`}
                >
                  <Icon size={15} />
                ...
                <>
                  ...
                  <div className="relative">
                    <button
                      ...
                      className={`flex min-h-10 items-center gap-2 rounded-xl px-2 py-1 font-body text-[13px] font-bold transition-colors hover:border-brand-gold/45 ${
                        isHomeHero
                          ? 'border border-white/45 bg-white/[0.22] text-brand-black backdrop-blur-[2px]'
                          : 'border border-border bg-white text-foreground'
                      }`}
                    >
                      <div className="w-7 h-7 rounded-full flex items-center justify-center bg-brand-gold/20 text-brand-gold">
            ...
            <button onClick={() => setMobileOpen(!mobileOpen)} className={`justify-self-end rounded-[10px] border p-2 xl:hidden ${isHomeHero ? 'border-white/45 bg-white/[0.22] text-brand-black' : 'border-border bg-white text-primary'}`} aria-label="Toggle menu">
