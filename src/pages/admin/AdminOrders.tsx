...
const AdminOrders = () => {
  const { getAllOrders, verifyOrderPayment, rejectOrderPayment, updateOrderStatus } = useProductStore();
  ...
  const filtered = orders.filter(o => statusFilter === 'all' || o.orderStatus === statusFilter);

  const totalRevenue = orders.filter(o => o.paymentStatus === 'paid').reduce((s, o) => s + o.totalAmount, 0);

  const handleVerify = (id: string) => { verifyOrderPayment(id); toast.success('Payment verified, order confirmed!'); };
  const handleReject = (id: string) => { rejectOrderPayment(id); toast.error('Payment rejected, order cancelled'); };
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      <p className="font-body text-[10px] text-muted-foreground">{o.id} • {o.userName} • {o.userPhone}</p>
                    </div>
                  ...
                  <div className="flex items-center gap-3 mt-2 flex-wrap">
                    ...
                    <span className="font-body text-[10px] text-muted-foreground">Qty: {o.quantity} • 📦 {o.shippingAddress}</span>
                  </div>
                  <div className="flex items-center gap-2 mt-3">
                    ...
                    {o.orderStatus === 'confirmed' && (
                      <button onClick={() => { updateOrderStatus(o.id, 'shipped'); toast.success('Marked as shipped'); }} className="px-3 py-1 rounded bg-brand-gold/10 text-brand-gold font-body text-xs hover:bg-brand-gold/20">Mark Shipped</button>
                    )}
                    {o.orderStatus === 'shipped' && (
                      <button onClick={() => { updateOrderStatus(o.id, 'delivered'); toast.success('Marked as delivered'); }} className="px-3 py-1 rounded bg-brand-green/10 text-brand-green font-body text-xs hover:bg-brand-green/20">Mark Delivered</button>
                    )}
                  </div>
                </div>
              </div>
