...
const BookingDetail = () => {
  ...
  const booking = getBookingsByUser(user?.id || '').find((b) => b.id === id);
  ...
  // Try to get listing details from localStorage
  const getListingDetails = () => {
    try {
      const key = `vvs_${booking.bookingType}s`;
      const partnerKey = `vvs_partner_${booking.bookingType}s`;
      const items = [
        ...JSON.parse(localStorage.getItem(key) || '[]'),
        ...JSON.parse(localStorage.getItem(partnerKey) || '[]'),
      ];
      return items.find((i: any) => i.id === booking.itemId) || null;
    } catch { return null; }
  };

  const listing = getListingDetails();
      ...
      <div className="min-h-screen bg-background pt-24 pb-16 px-4">
        <div className="container mx-auto max-w-4xl">
          ...
          <div className="bg-card rounded-2xl border border-border overflow-hidden mb-6">
            ...
            <div className="p-6">
              <p className="font-body text-xs text-brand-crimson font-medium mb-1">{booking.id}</p>
              <h1 className="font-heading text-2xl sm:text-3xl font-bold text-foreground mb-2">{booking.itemName}</h1>
              {listing?.location && (
                <p className="font-body text-sm text-muted-foreground flex items-center gap-1"><MapPin size={14} /> {listing.location}</p>
              )}
            </div>
          </div>
              ...
              {listing && (
                <div className="bg-card rounded-xl border border-border p-6">
                  <h3 className="font-heading text-lg font-semibold text-foreground mb-4">
                    {booking.bookingType.charAt(0).toUpperCase() + booking.bookingType.slice(1)} Details
                  </h3>
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {listing.amenities.map((a: string, i: number) => (
                            <span key={i} className="px-2 py-1 rounded-lg bg-secondary text-secondary-foreground text-xs">{a}</span>
                          ))}
                        </div>
                      </div>
                    )}
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {listing.features.map((f: string, i: number) => (
                            <span key={i} className="px-2 py-1 rounded-lg bg-brand-gold/10 text-brand-gold text-xs">{f}</span>
                          ))}
                        </div>
                      </div>
                    )}
                    {listing.capacity && <p><span className="text-muted-foreground">Capacity:</span> {listing.capacity} persons</p>}
                    {listing.vehicleType && <p><span className="text-muted-foreground">Vehicle:</span> {listing.vehicleType}</p>}
                    {listing.duration && <p><span className="text-muted-foreground">Duration:</span> {listing.duration}</p>}
                  </div>
                </div>
              )}
